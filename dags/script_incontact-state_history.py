...
dag = DAG(
    'script-incontact-state-history_v1', default_args=default_args, schedule_interval='05 02 * * *',
    dagrun_timeout=timedelta(hours=2))

t_run_incontact_state_history = BashOperator(
    task_id='run_incontact-state_history',
    bash_command=PYTHON_PATH + 'python ' + SCRIPT_PATH + 'incontact-state_history.py',
    retries=3,
    dag=dag,
    env={
...
t_email_completion = EmailOperator(
    task_id='email_incontact-state_history_complete',
    to=["sdcde@smiledirectclub.com"],
    retries=0,
    dag=dag,
    subject="Incontact State History Import Complete",
    html_content="<h3>The Incontact State History Import script has been completed.</h3>")
t_email_completion.set_upstream(t_run_incontact_state_history)
