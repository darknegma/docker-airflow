...
dag = DAG(
    'script-incontact-agents_v1', default_args=default_args, schedule_interval='05 02 * * *',
    dagrun_timeout=timedelta(hours=2))

t_run_incontact_agents = BashOperator(
    task_id='run_incontact-agents',
    bash_command=PYTHON_PATH + 'python ' + SCRIPT_PATH + 'incontact-agents.py',
    retries=3,
    dag=dag,
    env={
...
t_email_completion = EmailOperator(
    task_id='email_incontact-agents_complete',
    to=["sdcde@smiledirectclub.com"],
    retries=0,
    dag=dag,
    subject="Incontact Agents Import Complete",
    html_content="<h3>The Incontact Agents Import  script has been completed.</h3>")
t_email_completion.set_upstream(t_run_incontact_agents)
